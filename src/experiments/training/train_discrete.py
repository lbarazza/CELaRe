...
# the PPO implemntation in the train_discrete function was adapted from https://github.com/vwxyzjn/cleanrl
...
from experiments.config.config import ArgsDiscrete
from experiments.utils import make_env, save_checkpoint
from celare import AgentDiscrete, oscill_coef
import wandb

# Function implementing the training loop using PPO for discrete action spaces
def train_discrete(args, verbose=False):
    ...
    agent = AgentDiscrete(envs, n_codebook_vectors=args.num_codebook_vectors, latent_dim=args.latent_dim, detach=args.detach, alpha=args.alpha).to(device)
    ...
    rewards = torch.zeros((args.num_steps, args.num_envs)).to(device) ### to CHANGE
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            rewards[step] = torch.tensor(reward).to(device).view(-1) ### CHANGED
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, \
                    quant_loss, recons_loss, clust_entropy_loss, _, _ \
                ...
                # select coefficient based on the mode
        ...
        if args.save_model and (global_step // (args.checkpoint_interval * checkpoint_counter) >= 1):
            checkpoint_counter += 1
            save_checkpoint(agent, run_name, global_step, verbose=True)
...
# run experiments for discrete action space sweep
def run_discrete_sweep(config=None):
    with wandb.init(config=config):
        # Update the args with sweep parameters
        config = wandb.config

        args = tyro.cli(ArgsDiscrete)
        ...
        elif config.env_id == "Acrobot-v1":
            args.total_timesteps = 500_000
        elif config.env_id == "MountainCar-v0":
            args.total_timesteps = 3_000_000
            args.oscill_amp = 0.1
            args.oscill_freq = 1/40_000.0
        elif config.env_id == "LunarLander-v2":
            args.total_timesteps = 3_000_000

        # run the training
        train_discrete(args, verbose=True)
